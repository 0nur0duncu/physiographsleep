...
# ============================================================
# 6. Adaptive Loss Doğrulama
# ============================================================
print("\n" + "=" * 70)
print("6. Adaptive Loss Doğrulama")
print("=" * 70)

K, gamma, eps = config.train.adaptive_loss.K, config.train.adaptive_loss.gamma, 1e-4
print(f"   K={K}, gamma={gamma}, eps={eps}")

scenario_f1s = [
    ("v1 final", np.array([0.983, 0.406, 0.877, 0.780, 0.850])),
...
# update_adaptive_weights test
loss_fn.update_adaptive_weights(np.array([0.98, 0.40, 0.88, 0.78, 0.85]), K=K, gamma=gamma)
new_weight = loss_fn.focal.weight
print(f"\n   update sonrası focal weights: {[f'{w:.3f}' for w in new_weight.tolist()]}")
print(f"   sum = {new_weight.sum().item():.1f} (beklenen: 5.0)")
assert abs(new_weight.sum().item() - 5.0) < 0.01
print(f"   ✅ Adaptive loss doğrulandı")

# ============================================================
# 7. Freeze/Unfreeze Doğrulama
# ============================================================
print("\n" + "=" * 70)
print("7. Stage-Specific Freeze Doğrulama")
print("=" * 70)
...
# Stage A: Freeze decoder ve bazı heads
model2 = PhysioGraphSleep(config.model)

# Stage A freeze (trainer'daki mantık)
for param in model2.sequence_decoder.parameters():
    param.requires_grad = False
for param in model2.heads.boundary_head.parameters():
    param.requires_grad = False
for param in model2.heads.prev_head.parameters():
    param.requires_grad = False
for param in model2.heads.next_head.parameters():
    param.requires_grad = False

trainable_a = count_trainable(model2)
frozen_a = count_frozen(model2)

print(f"\n   Stage A:")
print(f"     Trainable: {trainable_a:,}")
print(f"     Frozen:    {frozen_a:,}")
print(f"     Toplam:    {trainable_a + frozen_a:,}")
assert trainable_a + frozen_a == counts["total"]
print(f"     ✅ Toplam eşleşiyor")

# Stage B: Freeze encoders
model3 = PhysioGraphSleep(config.model)
for param in model3.waveform_stem.parameters():
    param.requires_grad = False
for param in model3.spectral_encoder.parameters():
    param.requires_grad = False
